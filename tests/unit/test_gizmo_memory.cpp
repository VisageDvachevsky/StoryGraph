...
public:
  ...
  static int getLeakCount() { return allocationCount - deletionCount; }
};
...
public:
  TrackedGizmoHandle(QGraphicsItem* parent = nullptr) : QGraphicsEllipseItem(parent) {
    DeletionTracker::allocationCount++;
  }

  ~TrackedGizmoHandle() override { DeletionTracker::deletionCount++; }
};
...
TEST_CASE("NMTransformGizmo scale corner handles have adequate hit areas",
          "[gizmo][scale][hit_area]") {
  int argc = 0;
  char* argv[] = {nullptr};
  QApplication app(argc, argv);
