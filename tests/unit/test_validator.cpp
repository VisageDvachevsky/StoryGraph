...
TEST_CASE("ScriptError - Format includes severity and location", "[script_error]") {
  ScriptError error(ErrorCode::UndefinedCharacter, Severity::Error,
                    "Character 'Test' is not defined", SourceLocation(10, 5));
...
TEST_CASE("ScriptError - Format with file path", "[script_error]") {
  ScriptError error(ErrorCode::UndefinedCharacter, Severity::Error, "Undefined character 'Villian'",
...
TEST_CASE("ScriptError - errorCodeString and helpUrl", "[script_error]") {
  ScriptError error(ErrorCode::UndefinedCharacter, Severity::Error, "Test", SourceLocation(1, 1));
...
TEST_CASE("Validator - Missing scene object warning with callback", "[validator][resources]") {
  ...
  validator.setSceneObjectExistsCallback(
      [](const std::string& sceneId, const std::string& objectId) {
...
TEST_CASE("Validator - No warnings when resources exist", "[validator][resources]") {
  ...
  validator.setSceneFileExistsCallback([](const std::string&) { return true; });

  validator.setSceneObjectExistsCallback(
      [](const std::string&, const std::string&) { return true; });

  validator.setAssetFileExistsCallback([](const std::string&) { return true; });
