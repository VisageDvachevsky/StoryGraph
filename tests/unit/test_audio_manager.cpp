    ...
    for (int i = 0; i < 4; ++i) {
      threads.emplace_back([&]() {
        for (int j = 0; j < 10; ++j) {
          ...
          [[maybe_unused]] auto musicVolume = manager.getChannelVolume(AudioChannel::Music);
        }
      });
    }
    ...
    for (int i = 0; i < 4; ++i) {
      threads.emplace_back([&]() {
        for (int j = 0; j < 10; ++j) {
          ...
          [[maybe_unused]] auto muted = manager.isChannelMuted(AudioChannel::Sound);
        }
      });
    }
    ...
    for (int t = 0; t < 4; ++t) {
      threads.emplace_back([&, t]() {
        for (int i = 0; i < 20; ++i) {
          ...
          config.volume = 0.5f;
          config.priority = i;
          auto handle = manager.playSound("sound_" + std::to_string(t * 100 + i), config);
          if (handle.isValid()) {
            soundsPlayed++;
          }
