 ...
 * @brief Unit tests for AudioManager, specifically master fade volume
 * interpolation
 *
 * These tests verify the fix for incorrect master fade volume calculation
 * where the interpolation used += instead of proper linear interpolation (Issue
 * #175).
 */
...
TEST_CASE("AudioManager fadeAllTo parameter validation", "[audio][manager]") {
  ...
  SECTION("Fade target is clamped to valid range") {
    // Start a fade to verify clamping (we can't directly observe
    // m_masterFadeTarget but we can verify the system doesn't crash)
...
TEST_CASE("AudioManager not playing without initialization",
          "[audio][manager]") {
  AudioManager manager;
...
TEST_CASE("AudioManager handle validity", "[audio][manager]") {
  ...
  SECTION("Default handle is invalid") { REQUIRE_FALSE(handle.isValid()); }
...
//   BEFORE: m_masterFadeVolume += (m_masterFadeTarget - m_masterFadeVolume) *
//   t; AFTER:  m_masterFadeVolume = m_masterFadeStartVolume +
