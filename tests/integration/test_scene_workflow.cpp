...
public:
  ...
  void drawText([[maybe_unused]] const renderer::Font& font, const std::string& text,
...
TEST_CASE("Scene workflow - Multiple characters and dialogue", "[integration][scene][multi]") {
  ...
  auto* alice = graph.showCharacter("alice", "alice_sprite", CharacterObject::Position::Left);
  auto* bob = graph.showCharacter("bob", "bob_sprite", CharacterObject::Position::Right);
...
TEST_CASE("Scene workflow - Typewriter effect simulation", "[integration][scene][typewriter]") {
  ...
  auto* dialogue =
      graph.showDialogue("Narrator", "This is a long message that will be revealed slowly...");
  REQUIRE(dialogue != nullptr);
