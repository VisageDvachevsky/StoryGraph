...
void NMVoiceManagerPanel::setupToolBar() {
  ...
  connect(scanBtn, &QPushButton::clicked, this, &NMVoiceManagerPanel::onScanClicked);
  ...
  autoMatchBtn->setToolTip(tr("Automatically match voice files to dialogue lines"));
  connect(autoMatchBtn, &QPushButton::clicked, this, &NMVoiceManagerPanel::onAutoMatchClicked);
  ...
  connect(importBtn, &QPushButton::clicked, this, &NMVoiceManagerPanel::onImportClicked);
  ...
  connect(exportBtn, &QPushButton::clicked, this, &NMVoiceManagerPanel::onExportClicked);
  ...
  connect(openFolderBtn, &QPushButton::clicked, this, &NMVoiceManagerPanel::onOpenVoiceFolder);
...
void NMVoiceManagerPanel::setupFilterBar() {
  ...
  connect(m_filterEdit, &QLineEdit::textChanged, this, &NMVoiceManagerPanel::onFilterChanged);
  ...
  connect(m_localeFilter, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &NMVoiceManagerPanel::onLocaleFilterChanged);
  ...
  connect(m_characterFilter, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
  ...
  connect(m_statusFilter, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &NMVoiceManagerPanel::onStatusFilterChanged);
...
void NMVoiceManagerPanel::setupVoiceList() {
  ...
  m_voiceTree->setHeaderLabels({tr("Line ID"), tr("Speaker"), tr("Scene"), tr("Text Key"),
                                tr("Voice File"), tr("Takes"), tr("Duration"), tr("Status"),
                                tr("Tags")});
  ...
  connect(m_voiceTree, &QTreeWidget::itemClicked, this, &NMVoiceManagerPanel::onLineSelected);
  connect(m_voiceTree, &QTreeWidget::customContextMenuRequested, this, [this](const QPoint& pos) {
    auto* item = m_voiceTree->itemAt(pos);
    if (!item)
      return;

    QMenu menu;
    menu.addAction(tr("Assign Voice File"), this, &NMVoiceManagerPanel::onAssignVoiceFile);
    menu.addAction(tr("Clear Voice File"), this, &NMVoiceManagerPanel::onClearVoiceFile);
    menu.addSeparator();
    menu.addAction(tr("Add Take"), this, &NMVoiceManagerPanel::onAddTake);
    menu.addAction(tr("Set Active Take"), this, &NMVoiceManagerPanel::onSetActiveTake);
    menu.addSeparator();
    menu.addAction(tr("Set Status"), this, &NMVoiceManagerPanel::onSetLineStatus);
...
void NMVoiceManagerPanel::setupPreviewBar() {
  ...
  connect(m_playBtn, &QPushButton::clicked, this, &NMVoiceManagerPanel::onPlayClicked);
  previewLayout->addWidget(m_playBtn);

  m_stopBtn = new QPushButton(tr("Stop"), previewWidget);
  m_stopBtn->setEnabled(false);
  connect(m_stopBtn, &QPushButton::clicked, this, &NMVoiceManagerPanel::onStopClicked);
  ...
  connect(m_volumeSlider, &QSlider::valueChanged, this, &NMVoiceManagerPanel::onVolumeChanged);
...
void NMVoiceManagerPanel::setupMediaPlayer() {
  ...
  if (m_audioPlayer) {
    ...
    m_audioPlayer->setOnPlaybackStateChanged(
        [this]([[maybe_unused]] AudioPlaybackState state) { onPlaybackStateChanged(); });
    m_audioPlayer->setOnMediaStatusChanged(
        [this]([[maybe_unused]] AudioMediaStatus status) { onMediaStatusChanged(); });
...
void NMVoiceManagerPanel::updateVoiceList() {
  ...
  const QString charFilter = m_characterFilter && m_characterFilter->currentIndex() > 0
                                 ? m_characterFilter->currentText()
                                 : QString();
  const bool showUnmatched = m_showUnmatchedBtn && m_showUnmatchedBtn->isChecked();

  // Get status filter (0 = All, 1 = Missing, 2 = Recorded, 3 = Imported, 4 =
  // NeedsReview, 5 = Approved)
  const int statusFilterIndex = m_statusFilter ? m_statusFilter->currentIndex() : 0;
  const bool filterByStatus = (statusFilterIndex > 0);
  const NovelMind::audio::VoiceLineStatus statusFilter =
      filterByStatus ? static_cast<NovelMind::audio::VoiceLineStatus>(statusFilterIndex - 1)
  ...
  for (const auto& line : lines) {
    ...
    const NovelMind::audio::VoiceLineStatus status =
        localeFile ? localeFile->status : NovelMind::audio::VoiceLineStatus::Missing;
    ...
    if (!filter.isEmpty()) {
      ...
      const QString speakerLower = QString::fromStdString(line.speaker).toLower();
      const QString textKeyLower = QString::fromStdString(line.textKey).toLower();
  ...
  for (const auto& filtered : filteredLines) {
    ...
    if (localeFile && localeFile->duration > 0) {
      const qint64 durationMs = static_cast<qint64>(localeFile->duration * 1000);
...
void NMVoiceManagerPanel::onDurationChanged(qint64 duration) {
  ...
  if (m_durationLabel) {
    ...
    m_durationLabel->setText(
        QString("%1 / %2").arg(formatDuration(position)).arg(formatDuration(duration)));
  }
}
...
void NMVoiceManagerPanel::onPositionChanged(qint64 position) {
  ...
  if (m_durationLabel) {
    m_durationLabel->setText(
        QString("%1 / %2").arg(formatDuration(position)).arg(formatDuration(m_currentDuration)));
  }
}
...
void NMVoiceManagerPanel::onImportClicked() {
  QString filePath = NMFileDialog::getOpenFileName(this, tr("Import Voice Mapping"), QString(),
                                                   tr("CSV Files (*.csv)"));
  if (!filePath.isEmpty()) {
...
void NMVoiceManagerPanel::onExportClicked() {
  QString filePath = NMFileDialog::getSaveFileName(this, tr("Export Voice Mapping"),
                                                   "voice_mapping.csv", tr("CSV Files (*.csv)"));
  if (!filePath.isEmpty()) {
...
void NMVoiceManagerPanel::onStopClicked() {
  stopPlayback();
}
...
void NMVoiceManagerPanel::onAssignVoiceFile() {
  ...
  QString filePath = NMFileDialog::getOpenFileName(this, tr("Select Voice File"), voiceDir,
...
void NMVoiceManagerPanel::onExportTemplateClicked() {
  ...
  QString filePath = NMFileDialog::getSaveFileName(this, tr("Export VO Template"),
                                                   "vo_template.json", tr("JSON Files (*.json)"));
  ...
  if (!filePath.isEmpty()) {
    ...
    if (result.isOk()) {
      ...
      m_statsLabel->setText(
          tr("Failed to export template: %1").arg(QString::fromStdString(result.error())));
      QTimer::singleShot(3000, this, [this]() { updateStatistics(); });
    }
  }
}
...
void NMVoiceManagerPanel::onEditLineMetadata() {
  ...
  bool ok = NMVoiceMetadataDialog::getMetadata(
      ...
      QString::fromStdString(line->speaker), QString::fromStdString(line->scene), result,
...
void NMVoiceManagerPanel::onAddTake() {
  ...
  QString filePath = NMFileDialog::getOpenFileName(this, tr("Select Take Audio File"), voiceDir,
                                                   tr("Audio Files (*.ogg *.wav *.mp3 *.flac)"));
  ...
  if (!filePath.isEmpty()) {
    ...
    auto result =
        m_manifest->addTake(dialogueId.toStdString(), m_currentLocale.toStdString(), take);
    if (result.isOk()) {
      updateVoiceList();
      ...
      m_statsLabel->setText(
          tr("Failed to add take: %1").arg(QString::fromStdString(result.error())));
      QTimer::singleShot(3000, this, [this]() { updateStatistics(); });
    }
  }
}
...
void NMVoiceManagerPanel::onSetActiveTake() {
  ...
  QString selected = NMInputDialog::getItem(
      this, tr("Select Active Take"), tr("Choose which take to set as active:"), takeNames,
      static_cast<int>(localeFile->activeTakeIndex), false, &ok);

  if (ok) {
    int selectedIndex = static_cast<int>(takeNames.indexOf(selected));
    auto result = m_manifest->setActiveTake(dialogueId.toStdString(), m_currentLocale.toStdString(),
...
void NMVoiceManagerPanel::onSetLineStatus() {
  ...
  QStringList statuses = {tr("Missing"), tr("Recorded"), tr("Imported"), tr("Needs Review"),
                          tr("Approved")};

  bool ok;
  QString selected = NMInputDialog::getItem(
      this, tr("Set Status"), tr("Select status for this line:"), statuses, 0, false, &ok);
  ...
  if (ok) {
    ...
    auto result =
        m_manifest->setStatus(dialogueId.toStdString(), m_currentLocale.toStdString(), status);
    if (result.isOk()) {
      updateVoiceList();
      updateStatistics();
    }
...
void NMVoiceManagerPanel::onRecordingCompleted(const QString& lineId, const QString& locale) {
  if (VERBOSE_LOGGING) {
#ifdef QT_DEBUG
...
void NMVoiceManagerPanel::onFileStatusChanged(const QString& lineId, const QString& locale) {
  if (VERBOSE_LOGGING) {
#ifdef QT_DEBUG
...
void NMVoiceManagerPanel::updateRowStatus(int row, const QString& lineId, const QString& locale) {
  if (!m_voiceTree || !m_manifest || row < 0 || row >= m_voiceTree->topLevelItemCount()) {
    return;
  }
  ...
  std::string localeStr = locale.isEmpty() ? m_currentLocale.toStdString() : locale.toStdString();
...
void NMVoiceManagerPanel::setupManifestCallbacks() {
  ...
  m_manifest->setOnStatusChanged([this](const std::string& lineId, const std::string& locale,
