...
public:
  ...
  [[nodiscard]] Result<PackVerificationReport> verifyHeader(const u8* data, usize size);

  [[nodiscard]] Result<PackVerificationReport>
  verifyResourceTable(const u8* data, usize size, u64 tableOffset, u32 resourceCount);

  [[nodiscard]] Result<PackVerificationReport>
  verifyResource(const u8* data, usize size, u64 offset, usize resourceSize, u32 expectedChecksum);

  [[nodiscard]] Result<PackVerificationReport>
  verifyPackSignature(const u8* data, usize size, const u8* signature, usize signatureSize);
  [[nodiscard]] Result<PackVerificationReport> verifyPackSignatureStream(std::istream& stream,
                                                                         usize size,
                                                                         const u8* signature,
                                                                         usize signatureSize);
...
public:
  ...
  [[nodiscard]] Result<std::vector<u8>> decrypt(const u8* data, usize size, const u8* iv,
                                                usize ivSize, const u8* aad, usize aadSize);

  [[nodiscard]] static Result<Core::SecureVector<u8>> deriveKey(const std::string& password,
                                                                const u8* salt, usize saltSize);

  [[nodiscard]] static Result<std::vector<u8>> generateRandomIV(usize size = 16);
...
public:
  ...
  [[nodiscard]] Result<std::vector<u8>> readResource(const std::string& resourceId);

  [[nodiscard]] bool isOpen() const { return m_isOpen; }
  [[nodiscard]] PackVerificationResult lastVerificationResult() const { return m_lastResult; }
