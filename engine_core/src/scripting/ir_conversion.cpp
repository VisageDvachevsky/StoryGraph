...
Result<std::unique_ptr<IRGraph>> ASTToIRConverter::convert(const Program& program) {
...
NodeId ASTToIRConverter::convertStatement(const Statement& stmt, NodeId prevNode) {
  ...
  return std::visit(
      [this, prevNode, &stmt](const auto& stmtData) -> NodeId {
        ...
        if constexpr (std::is_same_v<T, ShowStmt>) {
          IRNodeType nodeType = (stmtData.target == ShowStmt::Target::Background)
          ...
          NodeId nodeId = createNodeAndConnect(IRNodeType::HideCharacter, prevNode);
          auto* node = m_graph->getNode(nodeId);
          node->setProperty("character", stmtData.identifier);
          node->setSourceLocation(stmt.location);
          return nodeId;
          ...
          NodeId nodeId = createNodeAndConnect(IRNodeType::Transition, prevNode);
          auto* node = m_graph->getNode(nodeId);
...
NodeId ASTToIRConverter::createNodeAndConnect(IRNodeType type, NodeId prevNode) {
...
std::unique_ptr<Statement> IRToASTConverter::convertNode(const IRNode* node,
                                                         const IRGraph& /*graph*/) {
  ...
  case IRNodeType::ShowCharacter: {
    ...
    return std::make_unique<Statement>(std::move(show), node->getSourceLocation());
  }
  ...
  case IRNodeType::HideCharacter: {
    ...
    return std::make_unique<Statement>(std::move(hide), node->getSourceLocation());
  }
  ...
  case IRNodeType::Dialogue: {
    ...
    return std::make_unique<Statement>(std::move(say), node->getSourceLocation());
  }
  ...
  case IRNodeType::PlayMusic: {
    ...
    return std::make_unique<Statement>(std::move(play), node->getSourceLocation());
  }
  ...
  case IRNodeType::Wait: {
    ...
    return std::make_unique<Statement>(std::move(wait), node->getSourceLocation());
  }

  case IRNodeType::Goto: {
    GotoStmt gotoStmt;
    gotoStmt.target = node->getStringProperty("target");
    return std::make_unique<Statement>(std::move(gotoStmt), node->getSourceLocation());
  }
...
std::unique_ptr<Expression> IRToASTConverter::convertToExpression(const IRNode* node,
                                                                  const IRGraph& graph) {
  ...
  if (exprType == "literal") {
    ...
    return std::make_unique<Expression>(std::move(literal), node->getSourceLocation());

  } else if (exprType == "identifier") {
    IdentifierExpr identifier;
    identifier.name = node->getStringProperty("name");
    return std::make_unique<Expression>(std::move(identifier), node->getSourceLocation());
    ...
    if (opStr == "+")
      ...
      binary.op = TokenType::NotEqual;
    else if (opStr == "<")
      binary.op = TokenType::Less;
    else if (opStr == "<=")
      binary.op = TokenType::LessEqual;
    else if (opStr == ">")
      binary.op = TokenType::Greater;
    else if (opStr == ">=")
      binary.op = TokenType::GreaterEqual;
    else if (opStr == "and")
      binary.op = TokenType::And;
    else if (opStr == "or")
      binary.op = TokenType::Or;
    else
      binary.op = TokenType::Error; // Fallback
    ...
    return std::make_unique<Expression>(std::move(binary), node->getSourceLocation());
    ...
    if (opStr == "-")
      ...
      unary.op = TokenType::Error; // Fallback
    ...
    return std::make_unique<Expression>(std::move(unary), node->getSourceLocation());
    ...
    return std::make_unique<Expression>(std::move(call), node->getSourceLocation());
    ...
    return std::make_unique<Expression>(std::move(property), node->getSourceLocation());
  }
...
void ASTToTextGenerator::generateStatement(const Statement& stmt, int /*indentLvl*/) {
  indent();
...
void ASTToTextGenerator::newline() {
  m_output += "\n";
}

void ASTToTextGenerator::write(const std::string& text) {
  m_output += text;
}
