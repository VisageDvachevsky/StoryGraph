...
Result<void> SecurePackReader::openPack(const std::string& path) {
  ...
  if (m_header.stringTableOffset < m_header.resourceTableOffset + resourceTableSize ||
  ...
  for (u32 i = 0; i < stringCount; ++i) {
    ...
    const u64 stringEnd = static_cast<u64>(offsets[i]) + static_cast<u64>(str.size());
  ...
  for (const auto& entry : entries) {
    ...
    if (absoluteOffset + entry.compressedSize > m_fileSize - detail::kFooterSize) {
      m_lastResult = PackVerificationResult::CorruptedData;
