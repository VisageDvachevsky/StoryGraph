...
void AudioManager::setDuckingParams(f32 duckVolume, f32 fadeDuration) {
  m_duckVolume.store(std::max(0.0f, std::min(1.0f, duckVolume)), std::memory_order_relaxed);
  m_duckFadeDuration.store(std::max(0.0f, fadeDuration), std::memory_order_relaxed);
}
